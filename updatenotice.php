...
<nav class="navbar navbar-default navbar-fixed-top">
    ...
    <h1><h1 class="text-center">Update Notice  <?php echo "("; echo $_SESSION['username']; echo ")";
...
if (isset($_POST['submit'])) {
 ...
 $name_file = $_FILES['photo']['name'];
   ...
   $pla= "UPDATE `board` SET `subject` = '$b', `notice` = '$c ', `file` = '$local_image' WHERE `board`.`id` = $idd;";
...
$upd="select * from board where id='$idd'";
$updrec=mysql_query($upd);

while (($row=mysql_fetch_array($updrec))) {
$subject=$row['subject'];
$notice=$row['notice'];
$file=$row['file'];
...
<div class="form-group green-border-focus">
  ...
  <textarea class="form-control"  rows="4" name="notice" value="<?php echo $notice; ?>"><?php echo $notice; ?>  </textarea>
</div>

<div class="form-group green-border-focus">
  <label for="exampleFormControlTextarea5">Document</label>
 <input type="file" name="photo" value="<?php echo $file; ?>"><input type="text" name="file"  value="<?php echo $file; ?>" disabled>
</div>
...
else
{
  ...
  echo "Place Not Access This Page Go To Home";
}
